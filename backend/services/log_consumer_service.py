...
class LogConsumerService(BaseService):
    ...
    async def _process_log_entry(self, log_entry: LogEntry) -> None:
        """Process individual log entry using asyncio.to_thread for database write"""
        try:
            ...
            # Write to database using asyncio.to_thread to prevent LibSQL Rust panics
            # This ensures database operations run in thread pool, not blocking async loop
            await asyncio.to_thread(self._insert_log_to_database, log_entry)
    ...
    def _insert_log_to_database(self, log_entry: LogEntry) -> None:
        ...
        try:
            ...
            # Insert to database using shared connection
            # The connection's execute() method has built-in retry logic and health checks
