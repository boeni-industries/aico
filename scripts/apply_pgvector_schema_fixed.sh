...
docker exec -e PGPASSWORD aico-postgres psql -U postgres -d aico -c "CREATE TABLE IF NOT EXISTS aico_core.conversation_segments (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, conversation_id TEXT NOT NULL, role TEXT NOT NULL, content TEXT NOT NULL, embedding vector(384) NOT NULL, timestamp TIMESTAMPTZ NOT NULL, metadata JSONB, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);"
...
docker exec -e PGPASSWORD aico-postgres psql -U postgres -d aico -c "CREATE TABLE IF NOT EXISTS aico_core.kg_node_embeddings (node_id TEXT PRIMARY KEY, embedding vector(384) NOT NULL, document TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);"
...
docker exec -e PGPASSWORD aico-postgres psql -U postgres -d aico -c "CREATE TABLE IF NOT EXISTS aico_core.kg_edge_embeddings (edge_id TEXT PRIMARY KEY, embedding vector(384) NOT NULL, document TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);"
