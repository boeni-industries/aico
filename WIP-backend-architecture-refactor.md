...
- [x] **Tenant-scope NATS subjects (✅ COMPLETE: Implemented transparent tenant scoping in `MessageBusClient._topic_to_subject()`; subjects follow `aico.<tenant_id>.<domain>.<action>` pattern; `user_id` kept in attributes to prevent subject explosion; JetStream streams updated with tenant-scoped wildcards `aico.*.conversation.>`, `aico.*.interaction.>`; backward compatible for single-tenant mode; see `docs/architecture/tenant-scoped-nats-implementation.md` for details)**
- [x] **Enforce streaming durability policy (✅ COMPLETE: Stream chunks published via ephemeral `publish()` to `CONVERSATION_STREAM`; final responses use `publish_durable()` with JetStream + outbox fallback; no accidental persistence of chunk traffic)**
- [x] **Prove ZMQ removal repo-wide (✅ COMPLETE: Zero ZMQ code usage.

### High
- [x] **Align conversation schema and lifecycle with doc (✅ COMPLETE: `conversations` + `conversation_messages` tables exist and used; status/title/updated_at implemented; conversation lifecycle endpoints added: GET /conversations (list with pagination), GET /conversations/{id} (get detail), PATCH /conversations/{id} (update title/status); repository methods list_by_user, count_by_user, count_by_conversation implemented; standardized pagination with limit/offset + items/total response)**
- [x] **Standardize error envelope on all non-2xx responses (✅ COMPLETE: `APIErrorResponse` schema enforced via global exception handlers in `backend/core/exception_handlers.py`; all HTTPException, RequestValidationError, and unhandled exceptions converted to `{error_code, message, request_id, details}` format; request_id middleware auto-generates/extracts x-request-id header; `raise_api_error()` helper available in `backend/api/errors.py` for consistent error raising; all responses include x-request-id header for tracing)**
- [x] **Normalize pagination contract to `limit`/`offset` + `items`/`total` for list endpoints (✅ COMPLETE: Created `PaginatedResponse[T]` generic schema in `backend/api/pagination.py`; standardized contract enforced: query params `limit` (1-100, default 50) + `offset` (>=0, default 0), response format `{items: T[], total: int, limit: int, offset: int}`; ALL legacy code removed - deprecated `Pagination` class deleted, `ConversationsListResponse` removed; conversation endpoints use `PaginatedResponse[ConversationListItem]`; admin endpoints updated: `AuditListResponse = PaginatedResponse[AuditEntry]`, `FailedAuthAttemptsResponse = PaginatedResponse[FailedAuthAttempt]`; all router responses changed from `{entries/attempts: [], pagination: {total_count}}` to `{items: [], total, limit, offset}`; aico-studio updated: `PaginatedResponse<T>` interface added, components changed from `.entries`/`.attempts` to `.items`; Flutter frontend verified compatible (doesn't use affected endpoints); system fails loudly on incorrect format - no backward compatibility)**
- [x] **Implement real `turn_number` and ordering guarantees (✅ COMPLETE: Added `turn_number INTEGER NOT NULL` column to `conversation_messages` table with unique constraint `(tenant_id, conversation_id, turn_number)`; implemented atomic turn number allocation via `get_next_turn_number()` using SELECT FOR UPDATE to prevent race conditions; repository automatically assigns sequential turn numbers on message creation; all queries now use `ORDER BY turn_number ASC/DESC` for deterministic ordering (no timestamp dependencies); added `list_after_turn()` method for catch-up queries; created `/conversations/{id}/messages/catchup?after_turn=N` endpoint for reliable message replay; removed all hardcoded `turn_number = 1` stubs from conversation engine and router; migration script created at `scripts/migrate_turn_number.sql` - **MANUAL STEP REQUIRED**: Run migration to backfill existing messages with sequential turn numbers based on created_at ordering; guarantees: no duplicate turn numbers, no gaps in sequence, deterministic replay regardless of timestamp collisions)**
- [x] Apply gateway-wide rate limiting consistently across REST (not only WS).
