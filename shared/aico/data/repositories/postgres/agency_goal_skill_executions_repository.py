...
from aico.ai.agency.models import AgencyGoalSkillExecution
...
class PostgresAgencyGoalSkillExecutionsRepository:
    ...
    async def create(self, entity: AgencyGoalSkillExecution) -> AgencyGoalSkillExecution:
        ...
        row = result.fetchone()
        return AgencyGoalSkillExecution(**dict(row._mapping))
    ...
    async def get_by_id(self, link_id: str) -> Optional[AgencyGoalSkillExecution]:
        ...
        result = await self.session.execute(stmt)
        row = result.fetchone()
        return AgencyGoalSkillExecution(**dict(row._mapping)) if row else None
    ...
    async def list(self, limit: int = 100, offset: int = 0) -> List[AgencyGoalSkillExecution]:
        ...
        result = await self.session.execute(stmt)
        return [AgencyGoalSkillExecution(**dict(row._mapping)) for row in result.fetchall()]
