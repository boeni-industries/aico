"""
SchedulerTasksRepository - PostgreSQL implementation
...
class PostgresSchedulerTasksRepository(Repository[SchedulerTask]):
    """PostgreSQL implementation of scheduler tasks repository."""
    ...
    async def count(self, filters: Optional[dict] = None) -> int:
        """Count scheduler tasks with optional filters."""
    ...
    async def get_enabled_tasks(self) -> List[SchedulerTask]:
        """Get all enabled scheduler tasks."""
        stmt = select(scheduler_tasks).where(
            scheduler_tasks.c.enabled == True
        ).order_by(scheduler_tasks.c.created_at.desc())
