...
class TestAgencyAPIEndpoints:
    ...
    async def test_list_policies(self, test_db):
        ...
        rule_id = f"test-policy-{datetime.now(UTC).timestamp()}"
        ...
        async with UnitOfWork(session_factory) as uow:
            policies = await uow.ethics_policy_rules.list(filters={"enabled": True})
        ...
        for policy in policies:
            ...
            assert policy.effect in ["allow", "allow_with_warning", "needs_consent", "block"]
            assert (policy.priority or 0) >= 0
    ...
    async def test_list_policies_filter_by_target_type(self, test_db):
        ...
        rule_id = f"test-policy-{target_type}-{datetime.now(UTC).timestamp()}"
        async with UnitOfWork(session_factory) as uow:
            await uow.ethics_policy_rules.create(
                EthicsPolicyRule(
                    rule_id=rule_id,
                    rule_name="Test Policy Filter",
        ...
        async with UnitOfWork(session_factory) as uow:
            policies = await uow.ethics_policy_rules.list(filters={"enabled": True, "target_type": target_type})

        # Assert
        assert len(policies) > 0
        for policy in policies:
            assert policy.target_type == target_type
    ...
    async def test_grant_consent(self, test_db, test_user):
        ...
        session_factory = await get_session_factory()
        consent_id = f"test-consent-{datetime.now(UTC).timestamp()}"
        now = datetime.now(UTC).isoformat()

        async with UnitOfWork(session_factory) as uow:
            await uow.consent_user_consents.create(
                ConsentUserConsent(
        ...
        async with UnitOfWork(session_factory) as uow:
            consent = await uow.consent_user_consents.get_by_id(consent_id)

        # Assert
        assert consent is not None
        assert consent.user_id == test_user
        assert consent.granted in (True, 1)
    
    async def test_list_consents(self, test_db, test_user):
        """Test listing consents for a user."""
        from aico.data.consent.models import ConsentUserConsent
        from aico.data.postgres.connection import get_session_factory
        from aico.data.uow import UnitOfWork

        session_factory = await get_session_factory()
        consent_id = f"test-consent-list-{datetime.now(UTC).timestamp()}"
        now = datetime.now(UTC).isoformat()
        ...
        async with UnitOfWork(session_factory) as uow:
            consents = await uow.consent_user_consents.list(filters={"user_id": test_user})

        # Assert
        assert len(consents) > 0
        assert any(c.consent_id == consent_id for c in consents)
    ...
    async def test_revoke_consent(self, test_db, test_user):
        ...
        session_factory = await get_session_factory()
        consent_id = f"test-consent-revoke-{datetime.now(UTC).timestamp()}"
        now = datetime.now(UTC).isoformat()
        ...
        async with UnitOfWork(session_factory) as uow:
            entity = await uow.consent_user_consents.get_by_id(consent_id)
            assert entity is not None
            entity.granted = 0
            entity.revoked_at = revoked_at
            entity.updated_at = revoked_at
            await uow.consent_user_consents.update(entity)
            await uow.commit()

        async with UnitOfWork(session_factory) as uow:
            result = await uow.consent_user_consents.get_by_id(consent_id)

        # Assert
        assert result is not None
        assert result.granted in (False, 0)
