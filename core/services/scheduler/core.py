...
try:
    ...
    from core.services.scheduler.tasks.base import BaseTask, TaskContext, TaskResult, TaskStatus
    from core.services.scheduler.cron import CronParser
    from core.services.scheduler.priority_queue import PriorityTaskQueue
    from core.services.scheduler.retry_manager import RetryManager, RetryTracker
except ImportError:
...
try:
    ...
    from core.services.scheduler.metrics import track_job
...
class TaskRegistry:
    ...
    def __init__(self, config_manager, db_connection):
        ...
        self.logger = get_logger("core.scheduler.task_registry")
...
class TaskExecutor:
    ...
    def __init__(self, config_manager, db_connection, container=None):
        ...
        self.logger = get_logger("core.scheduler.task_executor")
        # SchedulerService will be used via UoW instead of TaskStore
            ...
            try:
                ...
                async with UnitOfWork(session_factory) as uow:
                    ...
                    if run_key:
                        ...
                        mark_started = getattr(repo, "mark_started", None)
    ...
    def check_for_stuck_tasks(self) -> list:
        ...
        for task_id, start_time in self.task_start_times.items():
            ...
            if duration_seconds > stuck_threshold:
                ...
                try:
                    import asyncio
                    from core.services.scheduler_events import broadcast_scheduler_event
    ...
    async def _record_completion(self, task_id: str, execution_id: str, result: TaskResult, 
        ...
        try:
            ...
            async with UnitOfWork(session_factory) as uow:
                ...
                run_key = getattr(existing_execution, "run_key", None)
                if run_key:
                    repo = uow.scheduler_run_ledger
                    mark_completed = getattr(repo, "mark_completed", None)
                    if mark_completed is not None:
                        final_state = "completed"
                        status_value = status.value if hasattr(status, "value") else str(status)
                        if status_value == "failed":
                            final_state = "failed"
                        elif status_value in {"skipped", "deferred", "cancelled"}:
...
class TaskScheduler(BaseService if BACKEND_AVAILABLE else object):
    ...
    async def _reconcile_planned_runs(self, *, now: datetime) -> None:
        ...
        rolling_window_start = now - timedelta(seconds=backfill_seconds)
        rolling_window_end = now + timedelta(seconds=lookahead_seconds)
        utc_day_start = datetime(now.year, now.month, now.day, tzinfo=timezone.utc)
        utc_day_end = utc_day_start + timedelta(days=1)

        # Studio's scheduler timeline requests the full current UTC day. If we only
        # reconcile a narrow rolling window, the run ledger can look empty/partial
        # even though the scheduler is healthy. Keep the rolling window semantics,
        # but guarantee coverage for the whole current UTC day as well.
        window_start = min(rolling_window_start, utc_day_start)
        window_end = max(rolling_window_end, utc_day_end)
    ...
    async def _execute_task_with_retry(self, task_class: Type[BaseTask], prioritized_task):
        ...
        try:
            ...
            result = await self.task_executor.execute_task(
                ...
                retry_count=retry_count,
                run_key=prioritized_task.run_key
            )
            ...
            if result.success:
                ...
                if self.retry_manager.should_retry(retry_count, retry_config):
                    ...
                    success = self.priority_queue.enqueue(
                        ...
                        unique_key=prioritized_task.unique_key,
                        run_key=prioritized_task.run_key,
                        scheduled_for=prioritized_task.scheduled_for,
                        tenant_id=prioritized_task.tenant_id,
                    )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if not hasattr(self, 'task_registry') or self.task_registry is None:
            return {
                'running': False,
                'registered_tasks': 0,
                'scheduled_tasks': 0,
        ...
        if not hasattr(self, 'task_executor') or self.task_executor is None:
            return {
                'running': getattr(self, 'running', False),
                'registered_tasks': len(self.task_registry.tasks) if self.task_registry else 0,
                'scheduled_tasks': len(getattr(self, 'next_run_times', {})),
                'running_tasks': 0,
                'next_run_times': {
                    task_id: next_run.isoformat() 
                    for task_id, next_run in getattr(self, 'next_run_times', {}).items()
                },
                'tasks': []
            }
        ...
        return {
            ...
            'tasks': list(self.task_registry.tasks.keys()) if self.task_registry else []
        }
