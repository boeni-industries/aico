...
from gateway.api.agency.models import (
...
logger = get_logger("gateway.api.agency")
router = APIRouter()
...
def _lesson_to_api(lesson) -> LessonResponse:
    ...
    return LessonResponse(
        ...
        created_at=getattr(lesson, "created_at", datetime.now(timezone.utc)),
        updated_at=getattr(lesson, "updated_at", datetime.now(timezone.utc)),
    )
...
async def _set_cached(key: str, value: Any):
    ...
    _agency_cache[key] = (value, datetime.now(timezone.utc))
...
async def grant_consent(
    ...
    try:
        ...
        user_id = user["user_uuid"]
        consent_id = f"consent-{user_id}-{datetime.now(timezone.utc).timestamp()}"
    ...
    try:
        ...
        consent_list = [
            ConsentResponse(
                ...
                decision=c.decision,
                granted_at=c.granted_at if c.granted_at else datetime.now(timezone.utc)
            )
            for c in consents
        ]
...
async def get_agency_state(
    ...
    try:
        ...
        consent_required_actions: List[Dict[str, Any]] = []
        try:
            consent_rows = await uow.agency_events_log.list(
                filters={"user_id": user_id, "event_type": "curiosity_signal_needs_consent"},
                limit=50,
            )
            consent_events = [_event_log_to_api(row) for row in consent_rows]

            seen_consent_keys: set[str] = set()
            for event in sorted(consent_events, key=lambda e: e.created_at, reverse=True):
                metadata = event.metadata if isinstance(event.metadata, dict) else {}
                consent_scope = metadata.get("consent_scope") or {}
                consent_key = str(
                ...
                consent_required_actions.append(
                    {
                        ...
                        "policy_message": metadata.get("user_message"),
                        "source": event.source,
                    }
                )
        except Exception as consent_err:
            logger.error(
                f"[AGENCY_API] Failed to build consent_required_actions for user {user_id}: {consent_err}",
                exc_info=True,
            )
