...
class ProtocolAdapter(ABC):
    ...
    def __init__(self, config: Dict[str, Any], dependencies: Dict[str, Any]):
        ...
        self.logger = dependencies.get('logger', get_logger("gateway.core.protocol"))
        self.running = False
...
class ProtocolAdapterManager:
    ...
    def __init__(self, config: Dict[str, Any], logger=None):
        self.logger = logger if logger else get_logger("gateway.core.protocol_manager")
        self.config = config
        ...
        try:
            ...
            if name == "websocket":
                ...
                adapter = adapter_class(
                    ...
                    dependencies.get('rate_limiter'),
                    dependencies.get('validator'),
                    dependencies.get('security_middleware'),
                    dependencies.get('key_manager'),
                )
            else:
    ...
    async def start_adapter(self, name: str) -> bool:
        ...
        try:
            ...
            # Start adapter (all adapters now use parameterless start())
            await adapter.start()
