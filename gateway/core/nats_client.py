...
class GatewayNATSClient:
    ...
    async def request_update_kg_query_templates(self, user_id: str, templates: list[Dict[str, Any]]) -> Dict[str, Any]:
        """Request KG query template update from core via NATS"""
        payload = json.dumps({"user_id": user_id, "templates": templates}).encode("utf-8")
        return await self._nats_request_with_trace("kg.query-templates.update", payload)
    ...
    async def request_operations_backup_status(self, *, backup_id: str) -> Dict[str, Any]:
        payload = json.dumps({"backup_id": backup_id}).encode("utf-8")
        return await self._nats_request_with_trace("operations.backup.status", payload, timeout=10.0)

    async def request_operations_backup_download_url(self, *, backup_id: str, expires_seconds: int = 300) -> Dict[str, Any]:
        payload = json.dumps({"backup_id": backup_id, "expires_seconds": int(expires_seconds)}).encode("utf-8")
        return await self._nats_request_with_trace("operations.backup.download_url", payload, timeout=10.0)
    ...
    async def request_system_overview(self, user_id: str) -> Dict[str, Any]:
        """Request system overview from core via NATS"""
        payload = json.dumps({"user_id": user_id}).encode("utf-8")
        return await self._nats_request_with_trace("system.overview", payload)
    ...
    async def request_health_detailed(self) -> Dict[str, Any]:
        """Request detailed health payload from core via NATS"""
        return await self._nats_request_with_trace("system.health.detailed", b"{}")
